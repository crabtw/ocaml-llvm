...
(* OASIS_START *)
(* DO NOT EDIT (digest: a3bdcb48ea816a193d11df4ab9c63d7f) *)
(*
...
let setup_t =
  {
               ...
               Library
                 ({
                     ...
                     },
                   {
                      ...
                      bs_c_sources = ["llvm_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                      bs_cclib =
                        [(OASISExpr.EBool true, ["${llvm_ldflags}"])];
               ...
               Library
                 ({
                     ...
                     },
                   {
                      ...
                      bs_path = "analysis";
                      bs_compiled_object = Best;
                      bs_build_depends = [InternalLibrary "llvm"];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = ["llvm_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                   ...
                   {
                      lib_modules = ["Llvm_analysis"];
                      lib_pack = false;
                      lib_internal_modules = [];
                      lib_findlib_parent = Some "llvm";
                      lib_findlib_name = None;
                      lib_findlib_containers = [];
                      });
               Library
                 ({
                     ...
                     },
                   {
                      ...
                      bs_build_depends = [InternalLibrary "llvm"];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = ["bitreader_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                      bs_cclib =
                        [(OASISExpr.EBool true, ["${llvm_ldflags}"])];
               ...
               Library
                 ({
                     ...
                     },
                   {
                      ...
                      bs_build_depends = [InternalLibrary "llvm"];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = ["bitwriter_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                      bs_cclib =
                        [(OASISExpr.EBool true, ["${llvm_ldflags}"])];
               ...
               Library
                 ({
                     ...
                     },
                   {
                      bs_build = [(OASISExpr.EBool true, true)];
                      bs_install = [(OASISExpr.EBool true, true)];
                      bs_path = "executionengine";
                      bs_compiled_object = Best;
                      bs_build_depends =
                        [InternalLibrary "llvm"; InternalLibrary "target"];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = ["executionengine_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                      bs_cclib =
                        [(OASISExpr.EBool true, ["${llvm_ldflags}"])];
                   ...
                   {
                      lib_modules = ["Llvm_executionengine"];
                      lib_pack = false;
                      lib_internal_modules = [];
                      lib_findlib_parent = Some "llvm";
                      lib_findlib_name = None;
               ...
               Library
                 ({
                     ...
                     },
                   {
                      bs_build = [(OASISExpr.EBool true, true)];
                      bs_install = [(OASISExpr.EBool true, true)];
                      bs_path = "transforms/ipo";
                      bs_compiled_object = Best;
                      bs_build_depends = [InternalLibrary "llvm"];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = ["ipo_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                      bs_cclib =
                        [(OASISExpr.EBool true, ["${llvm_ldflags}"])];
               ...
               Library
                 ({
                     ...
                     },
                   {
                      bs_build = [(OASISExpr.EBool true, true)];
                      bs_install = [(OASISExpr.EBool true, true)];
                      bs_path = "transforms/scalar";
                      bs_compiled_object = Best;
                      bs_build_depends = [InternalLibrary "llvm"];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = ["scalar_opts_ocaml.c"];
                      bs_data_files = [];
                      bs_ccopt = [(OASISExpr.EBool true, ["${llvm_cflags}"])];
                      bs_cclib =
                        [(OASISExpr.EBool true, ["${llvm_ldflags}"])];
     ...
     oasis_version = "0.3.0";
     oasis_digest = Some "\235\140$\215\1810\147>z\021\167f\0067/\159";
     oasis_exec = None;
     oasis_setup_args = [];
     setup_update = false;
     };;
...
let exec bin args = OASISExec.run_read_one_line ~ctxt:!BaseContext.default bin args

let find_llvm_config = BaseCheck.prog_best "llvm_config" ["llvm-config"]

let check_llvm_ver bin = let open BaseSetup in
  BaseCheck.version "llvm" (OASISVersion.VGreaterEqual setup_t.package.version) (fun () ->
    let ver = exec bin ["--version"] in
    let len = try String.index ver 's' with Not_found -> String.length ver in
    String.sub ver 0 len
  )

let def_llvm_flags () =
  let llvm_config = find_llvm_config () in
  let llvm_ver = check_llvm_ver llvm_config () in
  let _ = BaseEnv.var_define "llvm_cflags" (fun () -> exec llvm_config ["--cflags"]) in
  let _ = BaseEnv.var_define "llvm_ldflags" (fun () ->
    let lib = "-lLLVM-" ^ llvm_ver in
    (exec llvm_config ["--ldflags"]) ^ lib
  ) in
  ()
